...
export class PhaserMapAdapter {
  ...
  handlePointerMove(pointer) { if (!this.artifact || this.dragging) return; const p = this.screenToWorld(pointer), ctx = this.game.canvas.getContext('2d'), hit = this.scene.model.regions.hitTest(p.x, p.y, ctx); if (hit !== this.scene.model.hovered) { this.scene.model.hovered = hit; this.controller.emit('region-hover', hit?.region || null); } }
  handlePointerUp(pointer) { if (this.dragging || pointer.rightButtonReleased?.() || pointer.getDistance?.() > 6) return; const p = this.screenToWorld(pointer), hit = this.scene.model.regions.hitTest(p.x, p.y, this.game.canvas.getContext('2d')); if (hit) this.selectRegion(hit.region.id); }
