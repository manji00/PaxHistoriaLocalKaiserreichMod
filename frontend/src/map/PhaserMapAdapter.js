...
export class PhaserMapAdapter {
  ...
  zoomBy(factor, pointer) { const c = this.scene.cameras.main, before = this.screenToWorld(pointer || { x: c.width / 2, y: c.height / 2 }); c.setZoom(Math.max(this.minZoom, Math.min(8, c.zoom * factor))); const after = this.screenToWorld(pointer || { x: c.width / 2, y: c.height / 2 }); c.scrollX += before.x - after.x; c.scrollY += before.y - after.y; this.cameraChanged(); }
  zoomIn() { this.zoomBy(1.25); } zoomOut() { this.zoomBy(.8); }
  handleWheel(pointer, dy) { this.zoomBy(dy < 0 ? 1.18 : .85, pointer); }
  ...
  handlePointerMove(pointer) { if (!this.artifact) return; const p = this.screenToWorld(pointer), ctx = this.game.canvas.getContext('2d'), hit = this.scene.model.regions.hitTest(p.x, p.y, ctx); if (hit !== this.scene.model.hovered) { this.scene.model.hovered = hit; this.controller.emit('region-hover', hit?.region || null); } }
  handlePointerUp(pointer) { if (pointer.getDistance?.() > 6) return; const p = this.screenToWorld(pointer), hit = this.scene.model.regions.hitTest(p.x, p.y, this.game.canvas.getContext('2d')); if (hit) this.selectRegion(hit.region.id); }
  selectRegion(id) { const view = this.scene.model.regions.views.get(String(id)); this.scene.model.selected = view || null; if (view) this.controller.emit('region-selected', view.region); }
  ...
  screenToWorld({ x, y }) { const c = this.scene.cameras.main; return { x: c.scrollX + x / c.zoom, y: c.scrollY + y / c.zoom }; }
